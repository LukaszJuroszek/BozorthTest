...
int main(int argc, char **argv)
{
	...
	{ /* -p probefile.xyt { gallery*.xyt | -G gallery.lis } */
	...
	if (probe_list != CNULL && fixed_gallery_file != CNULL)
	{   /* -P -g */
		/* print_probe_filename_only = 1; */
	}
	else if (gallery_list != CNULL && fixed_probe_file != CNULL)
	{   /* -G -p */
	...
	else
	{
		...
		if (both_from_argv)
		{ /* The command line will therefore contain (probes,gallery) xyt-filename pairs */
			if (argcount % 2 != 0)
			{
